...
**Route every subagent you spawn inside a playbook step through the `poteto-agent` definition** (code-writing delegates, ad-hoc helpers). `/poteto-mode` and `poteto-agent` route through the same wrapper. How you name it depends on the harness, and the definition itself is a per-harness file rather than a shared one:

| Harness | Definition | Spawn |
| --- | --- | --- |
| Claude Code | `~/.claude/agents/poteto-agent.md` | `subagent_type: "poteto-agent"` on the Agent tool |
| Cursor | the pstack plugin's `agents/` | the agent name on a Task call |
| OpenCode | `~/.config/opencode/agents/poteto-agent.md` | `@poteto-agent`, the task tool, or `opencode run --agent poteto-agent` |
| Pi | none | Pi core ships no subagent tool. Do not fabricate a Task call; run the step in the current session and say the fan-out collapsed to sequential. |

Substituting the harness's generic subagent (`generalPurpose`, `general`) skips the SKILL.md read and drifts. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `poteto-agent`.

**Model tiers are written `<model>/<effort>`.** `opus/xhigh` is super-high-effort judgment, `opus/high` is difficult but specified, `opus/medium` is coordination and delegation, `sonnet/high` executes a brief opus already wrote. The `Agent` tool accepts `model` only (`sonnet | opus | haiku | fable`), so the effort half is dropped there and the three opus tiers collapse to `model: "opus"`; never report a tier you did not actually pass. `Workflow`'s `agent()` takes `model` and `effort` separately, so panels that depend on tier separation belong there. Full role table: `~/.agents/pstack-models.md`.

**Panels need lens diversity, not tier diversity.** The four-model panels here existed because different vendors fail differently. On one family, four calls at four efforts largely agree. Give each panel member a distinct brief (correctness, security, performance, does-it-reproduce) instead of the same prompt at a different tier, or you are sampling one opinion four times and reporting it as four.

**Defaults for every `Task` call.** `run_in_background: true`, agent mode (readonly strips MCP), file pointers not inlined context, explicit model per role (configurable via `/setup-pstack`; defaults `sonnet/high` for code, `opus/xhigh` for prose and judgment). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (`opus/xhigh`) when the task needs judgment or the intent is vague, and to your strongest instruction-following model (`opus/high`) when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code model. Per-role lines in the `/setup-pstack` rule override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Task `model`).
