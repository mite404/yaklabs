...
3. Pick the runners. Use `arena runners` from `~/.agents/pstack-models.md` when present. Otherwise default to one each on `opus/xhigh`, `opus/high`, `sonnet/high`, `opus/medium`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
...
After all Phase B candidates complete, choose one model from the `arena cross-judge pool` in `~/.agents/pstack-models.md` when present. Otherwise use `opus/xhigh`, `opus/high`, `sonnet/high`, `opus/medium`. Prefer a different model family from the parent's. On an all-Claude roster that is unsatisfiable, so pick the pool entry whose tier is furthest from the parent's and **say in the synthesis note that the cross-judge shared the parent's family**, because agreement between two Opus calls is weaker evidence than agreement across vendors was. Do not reach for a paid external model here; that budget is reserved for `interrogate`. Spawn one readonly judge subagent on that model. It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
