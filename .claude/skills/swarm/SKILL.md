...
4. Pick the worker model from `swarm workers` in `~/.agents/pstack-models.md` when present. Otherwise use `sonnet/high`. For a model race, name each arm's model up front.
